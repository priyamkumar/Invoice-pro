...
router.get('/', auth, async (req, res) => {
  try {
    const { page = 1, limit = 20, status, startDate, endDate, clientId } = req.query;
    ...
    const invoices = await Invoice.find(query)
      .populate('clientId', 'name email gstin')
...
router.post('/', auth, [
  ...
  body('items.*.particulars').trim().isLength({ min: 1 }).withMessage('Particulars is required'),
  ...
  body('items.*.taxRate').isNumeric().withMessage('Tax rate must be a number'),
  body('companyInfo.name').trim().isLength({ min: 1 }).withMessage('Company name is required'),
...
router.get('/stats/overview', auth, async (req, res) => {
  try {
    ...
    res.json({
      total: stats[0] || { totalInvoices: 0, totalAmount: 0, paidAmount: 0, pendingAmount: 0 },
